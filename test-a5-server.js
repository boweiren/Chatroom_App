...
const initialize = data => {
    ...
    const tokenize = nameString => {
        let buffer = "",
            state = "";
        return Array.from(nameString).reduce((tokens, char, index, list) => {
            if (state === "m-obj") {
                if (char === "/") {
    ...
    const callObjectByString = (nameString, ...args) => {
        ...
        try {
            ...
            if (err instanceof TypeError && err.message === "Cannot read property 'call' of undefined") {
    ...
    let expressTests = {
        ...
        testMiddleware: (req, res) => {
            return new Promise((resolve, reject) => {
                ...
                setTimeout(() => reject(new Error('"next" was not called within the timeout duration')), 1e3)
            })
        },
            ...
            try {
                ...
                if (result instanceof Promise) {
                    new Promise((resolve, reject) => {
                        ...
                        setTimeout(() => reject(new Error("timed out on an asynchronous action")), 5e3)
                    }).then(val => sendJsonResult(res, val)).catch(err => res.status(500).send(err.message + "\n" + err.stack))
                } else {

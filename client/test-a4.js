...
window.addEventListener("load", () => {
    ...
    const safeFetch = (resource, init) => originalFetch(resource, init).then(resp => resp.status === 200 ? resp.text().then(text => text ? JSON.parse(text) : text) : resp.text().then(text => Promise.reject(new Error(text))));
    ...
    const makeTestMessage = () => ({
        username: selectRandom(words),
        text: Math.random().toString()
    });
    const makeTestRoom = () => new Room(Math.random().toString(), "Team " + selectRandom(words), __tester.defaults.image, [{
    ...
    const isEquivalent = (a, b) => {
        ...
        if (proto === Object.getPrototypeOf(b)) {
            if (proto === Boolean.prototype || proto === Number.prototype || proto === String.prototype) {
                return a === b
            } else if (proto === Array.prototype) {
                return a.length === b.length && a.reduce((acc, e, i) => acc && isEquivalent(e, b[i]), true)
            } else {
                let keys = Object.keys(a).sort();
                return isEquivalent(keys, Object.keys(b).sort()) && keys.reduce((acc, k, i) => acc && isEquivalent(a[k], b[k]), true)
            }
        } else return false
    };
    const tests = [{
                        ...
                        try {
                            ...
                            if (initialized) {
                                ...
                                try {
                                    ...
                                    let merged = rooms.map(room => Object.assign({
        ...
        run: async () => {
            ...
            try {
                ...
                let testConversation = makeTestConversation(__tester.defaults.testRoomId, Date.now() - 864e5);
                ...
                if (!conversation._id) {
                    ...
                    if (equiv) {
                        ...
                        try {
                            ...
                            conversation = await remoteFunc("callObjectByString", "db.getLastConversation", __tester.defaults.testRoomId, testConversation.timestamp + 2e3);
                            result.score += .25;
                            testConversation._id = _id;
                            let equiv = isEquivalent(testConversation, conversation);
                            if (equiv) {
                                result.score += .75;
                            ...
                            try {
                                ...
                                let testMessages = Array.from({
                                    length: blockSize - roomMessages.length
                                }, _ => Object.assign({
                                ...
                                if (!isEquivalent(testMessages, receivedMessages)) {
                                    ...
                                    if (added && added.messages instanceof Array) {
                                        if (!isEquivalent(testMessages, added.messages.slice(roomMessages.length).map(item => Object.assign(item, {
                                                roomId: __tester.defaults.testRoomId
                                            })))) {
                            ...
                            let response = await safeFetch("/chat/" + __tester.defaults.testRoomId + "/messages?before=" + (2e3 + testConversation.timestamp));
                            ...
                            if (!(Service.getLastConversation && Service.getLastConversation instanceof Function)) {
                                ...
                                let data = await Service.getLastConversation(__tester.defaults.testRoomId, 2e3 + testConversation.timestamp);
            ...
            if (!(makeConversationLoader && makeConversationLoader instanceof Function)) {
                ...
                if (Object.getPrototypeOf(makeConversationLoader) !== GeneratorFunction.prototype) {
                    ...
                    await (async () => {
                        ...
                        let testConversation = makeTestConversation(testRoom.id, defer.args.before - 36e5);
                ...
                if (Object.getPrototypeOf(testRoom.getLastConversation) !== makeConversationLoader.prototype) {
                    ...
                    try {
                        let roomId = await new Promise((resolve, reject) => {
                            ...
                            setTimeout(() => reject(new Error('Timed out after calling "room.getLastConversation.next" and waiting for "Service.getLastConversation" to be called')), 1e3)
                        });
                ...
                if (!mainScope["lobby"]) {
                    ...
                    try {
                        ...
                        if (!(testRoom.onFetchConversation && testRoom.onFetchConversation instanceof Function)) {
                            ...
                            if (chatView.chatElem.scrollTop !== chatView.chatElem.scrollHeight - scrollHeight) {
                                result.comments.push(printError('"scrollTop" has incorrect value: expected = ' + (chatView.chatElem.scrollHeight - scrollHeight) + ", current = " + chatView.chatElem.scrollTop))
                            } else {
                                result.score += .5;

window.addEventListener("load", () => {
    ...
    const tests = [{
        id: "1", description: "Base layout", maxScore: 6, run: async () => {
            let result = { id: 1, score: 0, comments: [] };

            let appView = document.querySelector("div#app-view");

            if (!appView) { result.comments.push(printError("Could not find div#app-view")) } else {
                result.score += 1;
                ...
                if (!appMenu) { result.comments.push(printError("Could not find ul#app-menu")) } else {
                    result.score += 1;
                    printOK("Found ul#app-menu");
                    let items = appMenu.querySelectorAll("li.menu-item");
                    if (items.length !== 2) { result.comments.push(printError("Could not find 2 li.menu-item elements")) } else {
                        result.score += 1;

                        printOK("Found 2 li.menu-item elements")
                    } let roomsLink = appMenu.querySelector('li a[href="/"]');

                    if (!roomsLink) { result.comments.push(printError("Could not find link to /")) } else {
                        result.score += 1;

                        printOK('Found the link to "/"')
                    } let profileLink = appMenu.querySelector('li a[href="/profile"]');

                    if (!profileLink) { result.comments.push(printError("Could not find link to /profile")) } else {
                        result.score += 1;
                ...
                if (!pageView) { result.comments.push(printError("Could not find div#page-view")) } else {
                    result.score += 1;
        ...
        id: "2", description: "index.html layout", maxScore: 6, run: async () => {
            ...
            if (!appView) { result.comments.push(printError("Could not find div#app-view")) } else {
                ...
                if (!pageView) { result.comments.push(printError("Could not find div#page-view")) } else {
                    ...
                    if (!pageContent) { result.comments.push(printError("Could not find div.content")) } else {
                        result.score += 1;

                        printOK("Found div.content");
                        let roomList = pageContent.querySelector("ul.room-list");
                        if (!roomList) { result.comments.push(printError("Could not find ul.room-list")) } else {
                            result.score += 1;
                            printOK("Found ul.room-list");
                            let chatLink = roomList.querySelector('li a[href="/chat"]');
                            if (!chatLink) { result.comments.push(printError("Could not find link to /chat")) } else {
                                result.score += 1;
                                printOK('Found the link to "/chat"')
                            }
                        } let pageControl = pageContent.querySelector("div.page-control");
                        if (!pageControl) { result.comments.push(printError("Could not find div.page-control")) } else {
                            result.score += 1;
                            printOK("Found div.page-control");
                            let roomInput = pageControl.querySelector("input[type=text]");
                            if (!roomInput) { result.comments.push(printError("Could not find text input in div.page-control")) } else {
                                result.score += 1;
                                printOK("Found a text input in div.page-control")
                            } let roomBtn = pageControl.querySelector("button");
                            if (!roomBtn) { result.comments.push(printError("Could not find button in div.page-control")) } else {
                                result.score += 1;
        ...
        id: "3", description: "chat.html layout", maxScore: 10, run: async () => {
            ...
            if (!appView) { result.comments.push(printError("Could not find div#app-view")) } else {
                ...
                if (!pageView) { result.comments.push(printError("Could not find div#page-view")) } else {
                    ...
                    if (!pageContent) { result.comments.push(printError("Could not find div.content")) } else {
                        ...
                        if (!roomName) { results.comments.push(printError("Could not find h4.room-name")) } else {
                            result.score += 1;
                            printOK("Found h4.room-name")
                        } let messageList = pageContent.querySelector("div.message-list");
                        if (!messageList) { result.comments.push(printError("Could not find div.message-list")) } else {
                            result.score += 1;
                            printOK("Found div.message-list");
                            let otherMessage = messageList.querySelector(".message:not(.my-message)");
                            if (!otherMessage) { result.comments.push(printError("Could not find at least 1 div.message that is not div.my-message")) } else {
                                result.score += 1;
                                printOK("Found a div.message that is not div.my-message");
                                let otherUsername = otherMessage.querySelector("span.message-user");
                                if (!otherUsername) { result.comments.push(printError("Could not find span.message-user inside div.message")) } else {
                                    result.score += .5;
                                    printOK("Found span.message-user inside div.message")
                                } let otherText = otherMessage.querySelector("span.message-text");
                                if (!otherText) { result.comments.push(printError("Could not find span.message-text inside div.message")) } else {
                                    result.score += .5;
                                    printOK("Found span.message-text inside div.message")
                                }
                            } let myMessage = messageList.querySelector(".my-message");
                            if (!myMessage) { result.comments.push(printError("Could not find at least 1 div.my-message")) } else {
                                result.score += 1;
                                printOK("Found a div.my-message");
                                let myUsername = myMessage.querySelector("span.message-user");
                                if (!myUsername) { result.comments.push(printError("Could not find span.message-user inside div.my-message")) } else {
                                    result.score += .5;
                                    printOK("Found span.message-user inside div.my-message")
                                } let myText = myMessage.querySelector("span.message-text");
                                if (!myText) { result.comments.push(printError("Could not find span.message-text inside div.my-message")) } else {
                                    result.score += .5;
                        ...
                        if (!pageControl) { result.comments.push(printError("Could not find div.page-control")) } else {
                            ...
                            if (!chatInput) { result.comments.push(printError("Could not find textarea in div.page-control")) } else {
                                printOK("Found a textarea in div.page-control");
                                result.score += 1
                            } let sendBtn = pageControl.querySelector("button");
                            if (!sendBtn) { result.comments.push(printError("Could not find button in div.page-control")) } else {
                                result.score += 1;
                                printOK("Found a button in div.page-control")
                            }
                        }
        ...
        id: "4", description: "profile.html layout", maxScore: 6, run: async () => {
            ...
            if (!appView) { result.comments.push(printError("Could not find div#app-view")) } else {
                ...
                if (!pageView) { result.comments.push(printError("Could not find div#page-view")) } else {
                    ...
                    if (!pageContent) { result.comments.push(printError("Could not find div.content")) } else {
                        ...
                        if (!form) { result.comments.push(printError("Could not find div.profile-form")) } else {
                            result.score += 1;
                            printOK("Found div.profile-form");
                            let fields = pageContent.querySelectorAll("div.form-field");
                            if (fields.length < 1) { result.comments.push(printError("Could not find at least 1 div.form-field inside div.profile-form")) } else {
                                result.score += 1;
                                printOK("Found a div.form-field inside div.profile-form");
                                let fieldLabel = fields[0].querySelector("label");
                                if (!fieldLabel) { result.comments.push(printError("Could not find label inside div.form-field")) } else {
                                    result.score += .5;
                                    printOK("Found label inside div.form-field")
                                } let fieldInput = fields[0].querySelector("input");
                                if (!fieldInput) { result.comments.push(printError("Could not find input inside div.form-field")) } else {
                                    result.score += .5;
                        ...
                        if (!pageControl) { result.comments.push(printError("Could not find div.page-control")) } else {
                            ...
                            if (!saveBtn) { result.comments.push(printError("Could not find button in div.page-control")) } else {
                                result.score += 1;
                                printOK("Found a button in div.page-control")
                            }
                        }
        ...
        id: "5.1", description: "CSS media query", maxScore: 4, run: async () => {
            ...
            if (!cssTag) {
                ...
                if (cssTag) { result.comments.push(printError("style.css is included using absolute URL")) }
            } else {
                result.score += 1;
            ...
            if (!appView) { result.comments.push(printError("Could not find div#app-view")) } else {
                ...
                } else { result.comments.push(printError("width of div#app-view is not 80% of document body when body width is greater than or equal to 1024")) } await new Promise((resolve, reject) => {
                ...
                } else { result.comments.push(printError("width of div#app-view is not 90% of document body when body width is greater than or equal to 768 and less than 1024")) } await new Promise((resolve, reject) => {
                ...
                } else { result.comments.push(printError("width of div#app-view is not 100% of document body when body width is less than 768")) }
            } return result
        }
    }, {
        id: "5.2", description: "Chat boxes styling", maxScore: 2, run: async () => {
            ...
            if (myMessage && otherMessage) {
                ...
                if (myRect.left > otherRect.left && myRect.right > otherRect.right) {
                    ...
                    if (myRect.left > otherRect.left && myRect.right > otherRect.right) {
                        result.score += 1;
                        printOK("My message box appears to be on the right side of friend's message box")
                    } else { result.comments.push(printError("My message box should be toward the right of friend's message box")) }
                ...
                if (myColor === otherColor) {
                    ...
                    if (myColor === otherColor) { result.comments.push(printError("My message box and friend's message box should have different background colors")) } else {
                        result.score += 1;
            ...
            } else { result.comments.push(printError("Could not find message boxes with classes .my-message and .message")) } return result
        }
    }, {
        id: "6", description: "CSS interactivity", maxScore: 1, run: async () => {
            let result = { id: 6, score: 0, comments: [] };
            let screenDevice;
            try { screenDevice = await navigator.mediaDevices.getDisplayMedia({ video: { cursor: { exact: "none" } }, audio: false }) } catch (err) {
                console.log(err);
            ...
            if (!appMenu) { result.comments.push(printError("Could not find ul#app-menu")) } else {
                ...
                if (!menuLink) { result.comments.push(printError("Could not find li.menu-item")) } else { await captureElemScreenshot(menuLink, screenDevice, result) }
            } let roomList = document.querySelector("ul.room-list");
            if (!roomList) { result.comments.push(printError("Could not find ul.room-list")) } else {
                let chatLink = roomList.querySelector("li");
                if (!chatLink) { result.comments.push(printError("Could not find ul.room-list li")) } else { await captureElemScreenshot(chatLink, screenDevice, result) }
    ...
    const captureElemScreenshot = async (elemToCapture, videoStream, result) => {
        ...
        await new Promise((resolve, reject) => {
            ...
            let blink = setInterval(() => { elemToCapture.style.border = elemToCapture.style.border === "3px dashed red" ? originalBorder : "3px dashed red" }, 500);
            let pixelPos = {};
            ...
            let handler = async evt => {
                ...
                if (originalPixel.data[0] !== hoverPixel.data[0] || originalPixel.data[1] !== hoverPixel.data[1] || originalPixel.data[2] !== hoverPixel.data[2] || originalPixel.data[3] !== hoverPixel.data[3]) {
                    result.score += .5;
                    printOK("Observed color change upon mouse hover")
                } else { result.comments.push(printError("Could not observe change in color on mouse hover")) } resolve(null)
            };
    ...
    const emoji = { bug: String.fromCodePoint(128030), like: String.fromCodePoint(128077) };
    ...
    else store = { options: { showLogs: true }, selection: {}, results: {}, lastTestAt: null };
    let ui = {};
    let dom = elem("div");
        ...
        await forEachAsync.call(tests, async test => {
            ...
            if (input.checked) {
                ...
                try {
                    ...
                    store.results[test.id] = { skipped: false, score: result ? Math.round(100 * result.score) / 100 : 0, comments: result ? result.comments : [] }
                } catch (err) {
                    store.results[test.id] = { skipped: false, score: 0, comments: ["Error while running tests: " + err.message] };
                    console.log(err)
                } if (store.options.showLogs) console.log("")
            } else { store.results[test.id] = { skipped: true, score: 0, comments: [] } } cell.textContent = store.results[test.id].skipped ? "Skipped" : Math.round(100 * store.results[test.id].score) / 100 + "/" + test.maxScore
        });

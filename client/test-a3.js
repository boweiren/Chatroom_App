const __tester = {
    ...
    timers: [],
    exports: new Map,
    defaults: {image: "assets/everyone-icon.png", webSocketServer: "ws://localhost:8000"},
    oldAddEventListener: HTMLElement.prototype.addEventListener,
    newAddEventListener: function (type, listener, ...options) {
        __tester.listeners.push({node: this, type: type, listener: listener, invoke: evt => listener.call(this, evt)});
    ...
    newSetInterval: function (func, delay, ...args) {
        __tester.timers.push({type: "Interval", func: func, delay: delay});
...
window.addEventListener("load", () => {
    ...
    const testServer = "http://99.79.42.146:3000/";
    const makeTestRoom = () => new Room(Math.random().toString(), Math.random().toString(), __tester.defaults.image, [{
        username: Math.random().toString(),
        text: Math.random().toString()
    }, {username: Math.random().toString(), text: Math.random().toString()}]);
    const remoteFunc = (func, ...args) => originalFetch("cpen322/" + a, {
        method: "POST",
        headers: {"Content-Type": "application/json"},
        body: JSON.stringify({func: func, args: args})
    }).then(resp => resp.status === 200 ? resp.json() : resp.text().then(text => {
        throw new Error(text)
    }));
    const tests = [{
        id: "1", description: "Getting data via AJAX GET", maxScore: 5, run: async () => {
            ...
            if (typeof Service === "undefined") {
                ...
                if (!(Service.getAllRooms && Service.getAllRooms instanceof Function)) {
                    ...
                    Service.origin = testServer + dataToSign;
                    try {
                    ...
                    Service.origin = testServer + "error/" + dataToSign;
                    try {
                        print('Calling "Service.getAllRooms" with "Service.origin" set to ' + Service.origin);
                        let promise = Service.getAllRooms();
                        if (!(promise && promise instanceof Promise)) {
                        ...
                        if (!(err instanceof Error && err.message === "Server Error 500: " + dataToSign)) {
                            //console.log("Server Error 500: " + dataToSign);
                    ...
                    dataToSign = Math.random().toString();
                    Service.origin = "http://invalid-url";
                    try {
                        print('Calling "Service.getAllRooms" with "Service.origin" set to ' + Service.origin);
                        let promise = Service.getAllRooms();
                        if (!(promise && promise instanceof Promise)) {
        ...
        id: "2", description: "Handling GET request at the Server", maxScore: 4, run: async () => {
        ...
        id: "3", description: "AJAX POST", maxScore: 4, run: async () => {
            ...
            if (typeof Service === "undefined") {
                ...
                if (!(Service.addRoom && Service.addRoom instanceof Function)) {
                    ...
                    Service.origin = testServer + dataToSign;
                    try {
                        let testRoom = {name: Math.random().toString(), image: Math.random().toString()};
                    ...
                    try {
                        ...
                        let testRoom = {name: Math.random().toString(), image: __tester.defaults.image};
                        print('Calling "Service.addRoom" with "Service.origin" set to ' + Service.origin);
            ...
            try {
                await new Promise((resolve, reject) => {
                    Service.addRoom = (async data => {
                        ...
                        return {id: testRoom.id, name: data.name, image: data.image}
                    });
        ...
        id: "4", description: "WebSocket Client", maxScore: 3, run: async () => {
                        ...
                        if (!(lobby instanceof Lobby)) {
                            ...
                            let promise = new Promise((resolve, reject) => {
                                Room.prototype.addMessage = ((username, text) => {
                                    ...
                                    if (username === testMessage.username && text === testMessage.text) {
                                        result.score += .5;
            ...
            if (typeof ChatView === "undefined") {
                ...
                let testObject = {value: Math.random()};
        ...
        id: "5", description: "WebSocket Server", maxScore: 4, run: async () => {
            let result = {id: 5, score: 0, comments: []};
            try {
                    ...
                    try {
                        ...
                        if (lengthAfter !== originalLength + 3) {
                            ...
                            if (messagesA.length === 0 && messagesB.length === 1 && messagesC.length === 1) {
                                if (!(messagesB[0].roomId === testMessage.roomId && messagesB[0].username === testMessage.username && messagesB[0].text === testMessage.text)) {
                                    result.comments.push(printError("Test client B did not receive the message sent by test client A"))
                                } else if (!(messagesC[0].roomId === testMessage.roomId && messagesC[0].username === testMessage.username && messagesC[0].text === testMessage.text)) {
                                    result.comments.push(printError("Test client C did not receive the message sent by test client A"))
                                } else if (messagesB[0].roomId === testMessage.roomId && messagesB[0].username === testMessage.username && messagesB[0].text === testMessage.text && messagesC[0].roomId === testMessage.roomId && messagesC[0].username === testMessage.username && messagesC[0].text === testMessage.text) {
                                    result.score += .5;
                            ...
                            if (messagesA.length === 1 && messagesB.length === 1 && messagesC.length === 2) {
                                if (!(messagesA[0].roomId === testMessage.roomId && messagesA[0].username === testMessage.username && messagesA[0].text === testMessage.text)) {
                                    result.comments.push(printError("Test client A did not receive the message sent by test client B"))
                                } else if (!(messagesC[1].roomId === testMessage.roomId && messagesC[1].username === testMessage.username && messagesC[1].text === testMessage.text)) {
                                    result.comments.push(printError("Test client C did not receive the message sent by test client B"))
                                } else if (messagesA[0].roomId === testMessage.roomId && messagesA[0].username === testMessage.username && messagesA[0].text === testMessage.text && messagesC[1].roomId === testMessage.roomId && messagesC[1].username === testMessage.username && messagesC[1].text === testMessage.text) {
                                    result.score += .5;
                            ...
                            if (messagesA.length === 2 && messagesB.length === 2 && messagesC.length === 2) {
                                if (!(messagesA[1].roomId === testMessage.roomId && messagesA[1].username === testMessage.username && messagesA[1].text === testMessage.text)) {
                                    result.comments.push(printError("Test client A did not receive the message sent by test client C"))
                                } else if (!(messagesB[1].roomId === testMessage.roomId && messagesB[1].username === testMessage.username && messagesB[1].text === testMessage.text)) {
                                    result.comments.push(printError("Test client C did not receive the message sent by test client A"))
                                } else if (messagesA[1].roomId === testMessage.roomId && messagesA[1].username === testMessage.username && messagesA[1].text === testMessage.text && messagesB[1].roomId === testMessage.roomId && messagesB[1].username === testMessage.username && messagesB[1].text === testMessage.text) {
                                    result.score += .5;
    ...
    if (store) store = JSON.parse(store); else store = {
